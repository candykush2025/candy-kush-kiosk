      ...
      for (const product of stockData.products) {
        try {
          ...
          if (product.variantId) {
            ...
            console.log(`Updating variant stock for product ${product.productId}, variant ${variantId}, option ${optionId}, quantity +${product.quantity}`);
            ...
            if (productDoc.exists()) {
              ...
              for (let i = 0; i < updatedVariants.length; i++) {
                if (updatedVariants[i].id === variantId) {
                  variantFound = true;
                  const updatedOptions = [...(updatedVariants[i].options || [])];
                  for (let j = 0; j < updatedOptions.length; j++) {
                    if (updatedOptions[j].id === optionId) {
                      ...
                      console.log(`Variant option stock update: ${oldQuantity} + ${product.quantity} = ${newQuantity}`);
                      updatedOptions[j] = {
                        ...updatedOptions[j],
              ...
              if (!variantFound) {
                console.warn(`Variant ${variantId} not found in product ${product.productId}`);
              } else if (!optionFound) {
                console.warn(`Option ${optionId} not found in variant ${variantId} of product ${product.productId}`);
              }
              ...
              console.log(`Successfully updated variant stock for product ${product.productId}`);
            } else {
              console.warn(`Product ${product.productId} not found`);
            }
          } else {
            // Handle simple product stock update
            console.log(`Updating simple product stock for ${product.productId}, quantity +${product.quantity}`);
            await updateDoc(productDocRef, {
              quantity: increment(product.quantity),
              updatedAt: Timestamp.now()
            });
            console.log(`Successfully updated simple product stock for ${product.productId}`);
          }
        } catch (productError) {
          console.error(`Error updating stock for product ${product.productId}:`, productError);
